...
export default function SetupPasswordPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    if (!token) {
      toast.error("Invalid setup link. Please contact the TnP office.");
      return;
    }
    try {
      await axios.post(`${API_BASE}/auth/setup-password`, {
        token,
      ...
      const message = error?.response?.data?.error || "Setup link is invalid or has expired.";
      toast.error(message);
    }
  });
        ...
        {done ? (
          <div className="rounded-[1.4rem] border border-outline-variant/30 bg-surface-container-lowest p-8 text-center space-y-4">
            ...
            <p className="text-sm leading-6 text-on-surface-variant">
              Your TNP Connect account is ready. Log in using your enrollment number and the
          ...
          <div className="rounded-[1.4rem] border border-outline-variant/30 bg-surface-container-lowest p-8 space-y-6">
            <motion.div custom={0} variants={fadeUp} initial="hidden" animate="show" className="space-y-1">
              ...
              <p className="text-sm leading-6 text-on-surface-variant">
                Choose a secure password for your TNP Connect account. You will use your enrollment
                number and this password to sign in.
              </p>
            </motion.div>

            {!token && (
              <div className="rounded-xl bg-error/10 px-4 py-3 text-sm text-error">
                This setup link is missing or invalid. Please contact the TnP office for a new link.
              </div>
            )}
